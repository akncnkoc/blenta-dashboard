...
} from '@/services/api/app-version-api'
...
export default function AppVersionScreen() {
  ...
  const [getAllAppVersions, { isLoading }] = useLazyGetAllAppVersionQuery()
  const [deleteAppVersion] = useDeleteAppVersionMutation()

  const [appVersions, setAppVersions] = useState<AppVersion[]>([])
  const [totalCount, setTotalCount] = useState(0)

  const [newAppVersionCode, setNewAppVersionCode] = useState('')
          ...
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
              ...
              <input
                ...
                value={newAppVersionCode}
                onChange={(e) => setNewAppVersionCode(e.target.value)}
              />
              <div className="flex justify-end gap-2">
                ...
                <button
                  onClick={async () => {
                    if (!newAppVersionCode.trim()) return
                    await createAppVersion({
                      version: newAppVersionCode.trim(),
                    })
                    setNewAppVersionCode('')
