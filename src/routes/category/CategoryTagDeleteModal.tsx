...
import { useDeleteCategoryTagMutation } from '@/services/api/tag-api'
...
export function CategoryTagDeleteModal({
  open,
  onOpenChange,
  categoryTagId,
}: Props) {
  const [deleteCategoryTag] = useDeleteCategoryTagMutation()

  const handleDelete = async () => {
    try {
      await deleteCategoryTag({ categoryTagId }).unwrap()
      toast.success('Category tag deleted successfully')
      onOpenChange(false)
    } catch {
      toast.error('Failed to delete category tag')
    }
  }
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-[50%] left-[50%] w-full max-w-md translate-x-[-50%] translate-y-[-50%] rounded-xl bg-white p-6 shadow-lg focus:outline-none">
          ...
          <Dialog.Description className="text-sm text-gray-600 mb-4">
            Are you sure you want to delete this category tag? This action
            cannot be undone.
          </Dialog.Description>
