...
} from '@/services/api/promotion-code-api'
import { Label } from '@/components/ui/label'

export default function PromotionCodeScreen() {
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const pageSize = 10

  const [getAllPromotionCodes, { isLoading }] =
    useLazyGetAllPromotionCodesQuery()
  const [updatePromotionCode] = useUpdatePromotionCodeMutation()
  const [deletePromotionCode] = useDeletePromotionCodeMutation()

  const [promotionCodes, setPromotionCodes] = useState<PromotionCode[]>([])
  const [totalCount, setTotalCount] = useState(0)

  const [editPromotionCode, setEditPromotionCode] =
    useState<PromotionCode | null>(null)
  const [editCode, setEditCode] = useState('')
  const [editExtraTime, setEditExtraTime] = useState('')
  const [newPromotionCodeCode, setNewPromotionCode] = useState('')
  const [newPromotionCodeExtraTime, setNewPromotionCodeExtraTime] = useState('')
  const [createDialogOpen, setCreateDialogOpen] = useState(false)

  const [createPromotionCode] = useCreatePromotionCodeMutation()
  ...
  useEffect(() => {
    getAllPromotionCodes({
      ...
      if (res?.data) {
        setPromotionCodes(res.data.data)
  ...
  const handleSaveEdit = async () => {
    if (editPromotionCode) {
      await updatePromotionCode({
        id: editPromotionCode.id,
        code: editCode,
        extraTime: editExtraTime,
      })
      setEditPromotionCode(null)
      getAllPromotionCodes({
        search,
        page: String(page),
        size: String(pageSize),
          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Portal>
              ...
              >
                <DropdownMenu.Item
                  onSelect={() => {
                    setEditPromotionCode(promotionCode)
                    setEditCode(promotionCode.code)
                    setEditExtraTime(promotionCode.extraTime)
                  }}
                ...
                <DropdownMenu.Item
                  onSelect={() => handleDelete(promotionCode.id)}
  ...
  return (
    <div className="p-4 w-full">
          ...
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
              ...
              <Dialog.Description className="text-sm text-gray-500 mb-4">
                Enter a code for the new promotion code.
              </Dialog.Description>
              <Label>Code</Label>
              <input
                className="w-full border px-3 py-2 rounded mb-4"
                value={newPromotionCodeCode}
                onChange={(e) => setNewPromotionCode(e.target.value)}
              />
              <Label>Extra Time</Label>
              <input
                className="w-full border px-3 py-2 rounded mb-4"
                value={newPromotionCodeExtraTime}
                onChange={(e) => setNewPromotionCodeExtraTime(e.target.value)}
              />
              <div className="flex justify-end gap-2">
                ...
                <button
                  onClick={async () => {
                    if (!newPromotionCodeCode.trim()) return
                    await createPromotionCode({
                      code: newPromotionCodeCode.trim(),
                      extraTime: newPromotionCodeExtraTime.trim(),
                    })
                    setNewPromotionCode('')
                    setNewPromotionCodeExtraTime('')
      ...
      <Dialog.Root
        open={!!editPromotionCode}
        onOpenChange={(open) => !open && setEditPromotionCode(null)}
      >
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 backdrop-blur-sm" />
          <Dialog.Content className="fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <Dialog.Description className="text-sm text-gray-500 mb-4">
              Modify the promotion code and save your changes.
            </Dialog.Description>
            <Label>Code</Label>
            <input
              className="w-full border px-3 py-2 rounded mb-4"
            ...
            <input
              className="w-full border px-3 py-2 rounded mb-4"
              value={editExtraTime}
              onChange={(e) => setEditExtraTime(e.target.value)}
            />
            <div className="flex justify-end gap-2">
              <Dialog.Close asChild>
                <button className="px-4 py-2 border rounded">Cancel</button>
